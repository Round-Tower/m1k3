...
- **mlx-swift-lm LLM generation** — the prior knowledge-server project only exercises `MLXEmbedders`; confirm `MLXLLM` from the same package runs Gemma 4 weights cleanly (Phase 2 first task). Ollama remains an emergency dev-only fallback if not.
