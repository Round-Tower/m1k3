...
class LlamaCppEngineTest {
    ...
    @Test
    fun `initialize calls backend init with resolved model path`() = runTest {
    ...
    @Test
    fun `initialize returns failure when backend returns zero handle`() = runTest {
    ...
    @Test
    fun `initialize is idempotent - only calls backend once`() = runTest {
    ...
    @Test
    fun `generate calls backend with stored handle and prompt`() = runTest {
    ...
    @Test
    fun `generate passes temperature from config`() = runTest {
        fakeBackend.initHandle = 1L
        engine.initialize()
    ...
    @Test
    fun `generate returns failure on empty response`() = runTest {
        fakeBackend.initHandle = 1L
    ...
    @Test
    fun `generate strips stop tokens from response`() = runTest {
        fakeBackend.initHandle = 1L
    ...
    @Test
    fun `generate returns metrics in result`() = runTest {
        fakeBackend.initHandle = 1L
    ...
    @Test
    fun `generateStreaming fails when not initialized`() = runTest {
    ...
    @Test
    fun `generateStreaming emits true streaming tokens via callback`() = runTest {
        fakeBackend.initHandle = 1L
        fakeBackend.generateResponse = "Hello world"
        ...
        val result = engine.generateStreaming("prompt", GenerationConfig()) { token ->
            received.add(token)
        }
    ...
    @Test
    fun `generateStreaming passes onToken callback to backend`() = runTest {
        fakeBackend.initHandle = 1L
    ...
    @Test
    fun `generateStreaming returns failure on empty response`() = runTest {
        fakeBackend.initHandle = 1L
        fakeBackend.generateResponse = ""
        engine.initialize()
    ...
    @Test
    fun `release calls backend release with stored handle`() = runTest {
    ...
    @Test
    fun `getOptimalMaxTokens returns 3072 for 8GB device`() {
        // deviceRamGbOverride = 8 → expected 3072
        assertEquals(3072, engine.getOptimalMaxTokens())
    }

    @Test
    fun `getOptimalMaxTokens returns 4096 for 12GB device`() {
        val highRamEngine = LlamaCppEngine(
            context = ApplicationProvider.getApplicationContext(),
            model = LlmModel.Gemma3_1B,
            overrideModelPath = tempModelFile.absolutePath,
            backend = fakeBackend,
        ...
        assertEquals(4096, highRamEngine.getOptimalMaxTokens())
    }

    @Test
    fun `getOptimalMaxTokens returns 1024 for sub-4GB device`() {
        val lowRamEngine = LlamaCppEngine(
            context = ApplicationProvider.getApplicationContext(),
            model = LlmModel.Gemma3_1B,
            overrideModelPath = tempModelFile.absolutePath,
            backend = fakeBackend,
        ...
        assertEquals(1024, lowRamEngine.getOptimalMaxTokens())
    }
}
